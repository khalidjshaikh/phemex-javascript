...
// Credentials DISABLED - using dummy values
const creds = { PHEMEX_API_KEY: "DISABLED", PHEMEX_API_SECRET: "DISABLED" };
const secretRaw = Buffer.from("DISABLED");
console.log(`[${new Date().toISOString().slice(0, 19).replace("T", " ")}]  ⚠  Trading DISABLED - credentials not loaded`);
...
const STATE_FILE = path.resolve(process.cwd(), `.vector-state-${SYMBOL}-v6.json`);
...
async function openLong(): Promise<void> {
  ...
  console.log(`[${tsNow()}]  📗  [TRADING DISABLED] would open LONG ${SYMBOL} size=${SIZE}`);
}

async function openShort(): Promise<void> {
  shortOpensHour.push(Date.now());
  console.log(`[${tsNow()}]  📕  [TRADING DISABLED] would open SHORT ${SYMBOL} size=${SIZE}`);
}

async function closePos(pos: Position): Promise<void> {
  console.log(`[${tsNow()}]  ✖  [TRADING DISABLED] would close ${pos.side} ${SYMBOL}`);
}
...
async function main(): Promise<void> {
  ...
  // }
  console.log(`[${tsNow()}]  ⚠  Leverage setting DISABLED`);

  // Close positions if requested [DISABLED]
  if (CLOSE_LONG || CLOSE_SHORT) {
    console.log(`[${tsNow()}]  ⚠  Position closing DISABLED`);
    // try {
