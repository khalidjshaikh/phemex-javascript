#!/usr/bin/env python3
"""
Phemex Price Slope Tracker

Tracks the slope (rate of change) of last price over multiple windows using River.
No trading - just prints slopes every tick for manual decision-making.

Usage:
  python3 phemex-river-trader2.py --symbol BTCUSDT --duration 0
"""
...
def parse_args():
    ...
    p.add_argument("--decimals", type=int, default=4, help="Decimal places for slope output (default: 4)")
    return p.parse_args()
...
async def run():
    ...
    header = f"{'Time':>12} {'Last':>12}" + "".join(f" {'Slope'+str(w):>12}" for w in WINDOWS)
    print(header)
    print("-" * len(header))
        ...
        try:
            async with websockets.connect(WS_URL, ping_interval=None) as ws:
                ...
                async for raw in ws:
                    ...
                    slopes = tracker.update(last_price)

                    ts = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                    slope_strs = "".join(f" {fmt_slope(slopes[w], args.decimals):>12}" for w in WINDOWS)

                    print(f"{ts:>12} {fmt(last_price, args.decimals):>12}{slope_strs}", flush=True)
