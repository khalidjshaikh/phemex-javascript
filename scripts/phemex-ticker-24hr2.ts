 ...
 *   npx tsx phemex-ticker-24hr.ts --velocity       # add vAsk/vBid/... = Δ per second
 */
...
Options:
  ...
  --velocity          Add vAsk, vBid, vIndex, vMark, vLast columns showing
                      each field's per-second rate of change
...
// --velocity: fields for which a velocity column (per-second rate of change
// since the previous tick, measured against the ticker's own timestamp) is added.
const VELOCITY = hasFlag("--velocity");
const VELOCITY_FIELDS = ["askRp", "bidRp", "indexRp", "lastRp", "markRp"] as const;
...
const COLUMN_ORDER = [
  ...
  "askRpVel", "bidRpVel", "indexRpVel", "markRpVel", "lastRpVel",
];
...
const COLUMNS: Record<string, { label: string; full: string }> = {
  ...
  // Velocity columns (--velocity): per-second rate of change of the field,
  // measured against the ticker's own timestamp delta.
  askRpVel:          { label: "vAsk",    full: "ask velocity (Δ/s)" },
  bidRpVel:          { label: "vBid",    full: "bid velocity (Δ/s)" },
  indexRpVel:        { label: "vIndex",  full: "index velocity (Δ/s)" },
  lastRpVel:         { label: "vLast",   full: "last velocity (Δ/s)" },
  markRpVel:         { label: "vMark",   full: "mark velocity (Δ/s)" },
};
...
function fmtField(k: string, v: unknown): string {
  ...
  if (k.endsWith("Delta")) return fmtDelta(v);
  if (k.endsWith("Vel")) return fmtVel(v);
  return fmt(v);
}

/**
 * Format a velocity value with a sign so columns align: "+0.0123", "-0.0002",
 * " 0.0000" (a space holds the sign position for zero).
 */
function fmtVel(v: unknown): string {
  if (v == null) return "—";
  const n = Number(v);
  if (!Number.isFinite(n)) return String(v);
  const s = n.toFixed(4);
...
async function main(): Promise<void> {
  ...
  let lastSig = "";
  // Previous tick's data and timestamp (ns) — used for the velocity columns.
  let prevData: Record<string, unknown> | undefined;
  let prevTsNs: number | undefined;
  ...
  for (;;) {
    ...
    try {
      ...
      // --velocity: append velocity columns, each field's per-second change
      // measured against the ticker's own timestamp delta.
      if (VELOCITY && prevData && Number.isFinite(prevTsNs)) {
        const tsNs = Number(data.timestamp);
        const dt = Number.isFinite(tsNs) ? (tsNs - prevTsNs!) / 1e9 : NaN;
        for (const f of VELOCITY_FIELDS) {
          const v = Number(data[f]);
          const p = Number(prevData[f]);
          data[`${f}Vel`] =
            dt > 0 && Number.isFinite(v) && Number.isFinite(p) ? (v - p) / dt : null;
        }
      }
      // Remember this tick for the next velocity computation.
      prevData = data;
      prevTsNs = Number(data.timestamp);
      ...
      fs.writeFileSync(INDEX_FILE, fmtExact(index), "utf8");
      fs.writeFileSync(INDEX_LAST_FILE, fmtExact(Number.isFinite(index) && Number.isFinite(last) ? index - last : null), "utf8");
      fs.writeFileSync(LAST_FILE, fmtExact(last), "utf8");
      fs.writeFileSync(MARK_FILE, fmtExact(mark), "utf8");
      fs.writeFileSync(MARK_LAST_FILE, fmtExact(Number.isFinite(mark) && Number.isFinite(last) ? mark - last : null), "utf8");
