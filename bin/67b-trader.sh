...
scripts/phemex-ws-auto-trader2.ts --no-reversal-exit "$@" --symbols XRPUSDT --credential 67b --configfile config/67b.json5 --no-ticker-logs | tee -a "logs2/trader-$(date '+%Y-%m-%d_%H-%M-%S').log"
