...
scripts/phemex-ws-auto-trader2.ts --no-reversal-exit "$@" --symbols XAUUSDT,XBRUSDT,XTIUSDT,XRPUSDT,XAUUSDT7 --credential meta --configfile config/gm.json5 | tee -a "logs/trader-$(date '+%Y-%m-%d_%H-%M-%S').log"
