...
scripts/phemex-ws-auto-trader2.ts --no-reversal-exit --error-budget 6 "$@" --symbol XAUUSDT,XBRUSDT,XTIUSDT --credential meta --configfile config/gm.json5 | tee -a "logs/trader-$(date '+%Y-%m-%d_%H-%M-%S').log"
