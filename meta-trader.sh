...
scripts/phemex-ws-auto-trader2.ts --no-reversal-exit "$@" --symbols XAUUSDT,XBRUSDT,XTIUSDT,XRPUSDT --credential meta --configfile config/gm.json5 | tee -a "logs/trader-$(date '+%Y-%m-%d_%H-%M-%S').log"
