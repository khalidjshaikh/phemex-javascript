 ...
 * npu.ts
 *
 * Library: adds two numbers on the Apple Neural Engine (NPU) via the ONNX
 * Runtime CoreML execution provider, using the model.onnx graph C = A + B.
 * Consumed by npu-app.ts (mirrors src/gpu.js for the GPU app).
 */
...
/** model.onnx lives at the repo root, next to the apps that use this library. */
...
export async function add(a: number, b: number): Promise<number> {
  console.log(`Adding ${a} + ${b} on the NPU...`);
